...
# original code in a Chinese forum: http://cos.name/bbs/read.php?tid=16743
# this code was written to celebrate China's 60 anniversary
if (!require("rgl")) stop("You need the rgl package to generate the 3D heart!") else {
    ...
    fz = function(z, x, y, w = 0, tt = 0) {
        (x^2 + 2 * y^2 + z^2 + 0.1 * cos(w * tt) - 0.9)^3 - (x^2 + 
            y^2/9) * z^3
    }
    n = 100
    ...
    for (i in 1:length(y)) {
        ...
        solvex = function(theta, y) {
            if (theta == -pi/2 | theta == pi/2 | theta == 1.5 * 
                pi) {
                return(0)
            }
        ...
        z[x == 0] = (theta[x == 0] == pi/2) * zplus + (theta[x == 
            0] != pi/2) * zminus
        xx = cbind(xx, x)
        zz = cbind(zz, z)
    }
    ...
    library(rgl)
    persp3d(zz, xx, yy, col = "red", xlim = c(-1, 1), ylim = c(-1, 
        1), zlim = c(-1, 1), axes = FALSE, box = FALSE, xlab = "", 
        ylab = "", zlab = "")
    fy = function(y, pars) {
        z = pars[1]
        x = pars[2]
    ...
    gety = function(z, x, interval = c(0.01, 1), w = 0, tt = 0) {
        ...
        solvey = function(pars) {
            if (fy(interval[1], pars) * fy(interval[2], pars) > 
                0) {
                return(NA)
            }
            else {
    ...
    persp3d(x = z0, y = x0, z = y0, zlim = c(-1, 1), col = "white", 
        texture = system.file("img", "flag.png", package = "fun"), 
        add = TRUE)
    persp3d(x = z0, y = x0, z = -y0, zlim = c(-1, 1), col = "red", 
        add = TRUE)
} 
